...
export default function DisplayScreen() {
  ...
  const formatGameTime = (ms) => {
    ...
    if (totalSeconds >= 60) {
      ...
      // under 1 min → show seconds + tenths
      const seconds = (ms / 1000).toFixed(1);
      return seconds;
    }
  };

  // Always format rest as MM:SS (even < 60s)
  ...
  const getCurrentRest = () => {
    ...
    const elapsed = Date.now() - (scoreboardData.restLastUpdate || scoreboardData.lastUpdate || Date.now());
  ...
  const renderFoulLights = (count) => {
    // Use scoreboardData.foulLimit if available, otherwise default to 5.
    const foulLimit = Number(scoreboardData.foulLimit ?? 5);
    const maxLights = 6; // visual cap
    const filled = Math.min(count, maxLights);
    const lights = [];
    for (let i = 0; i < maxLights; i++) {
      const isFilled = i < filled;
      // BONUS shown only when strictly greater than foulLimit (i.e., after five lights if default is 5)
      const inBonus = count > foulLimit;
      lights.push(
          <div
              key={i}
              aria-hidden
              className={`w-6 h-6 rounded-full border-2 ${isFilled ? "bg-red-500 border-red-600" : "bg-transparent border-gray-600"} ${inBonus && isFilled ? "ring-2 ring-yellow-400" : ""}`}
        ...
        <div className="flex gap-2 items-center justify-center">
          ...
          {count > foulLimit && (
              <div className="ml-4 text-yellow-300 font-bold flex items-center gap-2">
                <span className="px-2 py-1 rounded bg-yellow-600 text-black">BONUS</span>
  ...
  return (
      <div className="min-h-screen bg-gradient-to-b from-gray-900 to-black text-white p-4 flex justify-center items-center">
        ...
        <div className="max-w-6xl w-full bg-black rounded-2xl border-8 border-yellow-500 p-8 shadow-2xl">
          {/* Header */}
          <div className="flex justify-between items-center mb-8">
            <div className="text-4xl font-bold">Q{scoreboardData.quarter}</div>

            {/* center: rest OR shot clock */}
            {scoreboardData.restActive ? (
                <div className="text-center px-6 py-4 bg-blue-700/90 rounded-2xl shadow-md">
                  <div className="text-sm uppercase tracking-wider text-blue-100">Rest</div>
                  <div className="text-3xl md:text-4xl font-mono font-extrabold text-white mt-1">{formatRestTime(restTime)}</div>
                  <div className="text-xs text-blue-100/80 mt-1">break between quarters</div>
                </div>
            ) : (
                <div
                    className={`px-6 py-3 rounded-2xl text-center shadow-lg ${shotTime < 5000 ? "bg-red-700" : "bg-red-600"}`}
                >
                  <div className="text-sm">SHOT CLOCK</div>
                  <div className="text-5xl md:text-6xl font-mono font-extrabold">{formatShotClock(shotTime)}</div>
                </div>
            )}

            <div className="text-right">
              <div className={`text-sm ${gameTime < 60000 ? "text-yellow-300" : "text-gray-300"}`}>Game</div>
              <div className={`text-5xl font-mono font-extrabold px-6 py-2 rounded ${gameTime < 60000 ? "bg-yellow-600 text-black" : ""}`}>
                {formatGameTime(gameTime)}
              </div>
              {/* textual possession for clarity */}
              <div className="mt-2 text-sm text-gray-300">Possession: <span className="font-bold">{scoreboardData.possession === "A" ? scoreboardData.teamAName : scoreboardData.teamBName}</span></div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-12">
            {/* Team A (left) */}
            <div className="text-center relative" style={{ color: scoreboardData.teamAColor }}>
              {/* possession arrow: middle-right of left container */}
              ...
              <div className="text-5xl font-bold mb-4">{scoreboardData.teamAName}</div>
              ...
              <div className="bg-gray-800 p-4 rounded-lg shadow inline-block">
                ...
                <div className="text-4xl font-bold">{scoreboardData.teamATimeouts}</div>
              </div>
            </div>

            {/* Team B (right) */}
            <div className="text-center relative" style={{ color: scoreboardData.teamBColor }}>
              {/* possession arrow: middle-left of right container */}
              ...
              <div className="text-5xl font-bold mb-4">{scoreboardData.teamBName}</div>
              ...
              <div className="bg-gray-800 p-4 rounded-lg shadow inline-block">
                <div className="text-sm">TO</div>
                <div className="text-4xl font-bold">{scoreboardData.teamBTimeouts}</div>
              </div>
            </div>
          </div>

          {/* NOTE: removed the big "TIMEOUT" panel here per your request — we only display TO counts */}
        </div>
      </div>
  );
}
